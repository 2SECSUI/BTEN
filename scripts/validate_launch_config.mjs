...
if (lp.allocation.redirectedEmissionBps !== 3000 || lp.allocation.protocolOwnedLiquidityBps !== 7000 || lp.allocation.cetusNativeRewardsBps !== 3000) fail('LP allocation policy does not match the approved 30% / 70:30 split');
if (lp.allocation.protocolOwnedLiquidityBps + lp.allocation.cetusNativeRewardsBps !== 10_000) fail('LP split must sum to 10,000 bps');
...
for (const pool of lp.pools) if (!address.test(pool.poolId) || pool.weightBps <= 0 || pool.cetusRewardProgram !== null) fail(`invalid or prematurely-enabled pool configuration: ${pool.pair}`);
